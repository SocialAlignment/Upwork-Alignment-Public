...
import { analyzeProfile, generateProjectSuggestions, generatePricingSuggestions, generateGallerySuggestions, generateProcessSuggestions } from "./ai-service";
...
export async function registerRoutes(
  ...
  app.post("/api/process-suggestions", async (req, res) => {
    try {
      const { analysisData, projectIdea, projectTitle, projectCategory, pricingData } = req.body;
      ...
      const suggestions = await generateProcessSuggestions(
        analysisData, 
        projectIdea, 
        projectTitle,
        projectCategory || "General",
      ...
      res.status(500).json({ 
        error: "Failed to generate process suggestions. Please try again." 
      });
    }
  });
